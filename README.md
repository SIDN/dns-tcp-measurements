...
```
This is in the following order: `time,qname,qtype,prot,DO`. The time in seconds that the query should be sent in relation to the replay's start is denoted by `offset`. `DO` is the value of the DO-bit in the query. The other types are pretty straightforward, so if it should use UDP or TCP, what domain name it should request and what type of record it should request. 

## Build and usage
Build with `go install gitlab.sidnlabs.nl/eline.stehouwer/querier`, then run with `~/go/bin/querier`. \
Before running the code, you must also have an authoritative DNS nameserver running that you run the code against, 
...
As already mentioned, we have run our query replayer against 4 different nameserver implementations, and for each of them we have the 
...
```
In this command, we specify that the network used must be `host`. This is because the `slirp4netns` network is to be slower and is less able to handle many TCP requests compared to the `pasta` and `host` network. The `host` network can handle TCP requests a lot faster than the `pasta` network. \
...
#### BIND
To get the BIND implementation up and running, we use the official BIND9 container published by the ISC. So, to use it, we first need to pull 
it from the container hub, which can be done as follows:
```
podman pull docker.io/internetsystemsconsortium/bind9:9.20
```
The configuration file and folder structure needed for the BIND nameserver is located in the bind folder (with the configuration file located in `bind/etc/bind`). The configuration file needs to be changed to the settings that you prefer. The image also has to be able to access the zone file it is going to serve. You have to change the configuration file to point to the right path in the image where you will put the zone file. 
...
#### PowerDNS
Although we also wanted to use a container on the docker hub for the PowerDNS implementation, these did not work as we expected, so we had to do something else. We instead built our own container from a published PowerDNS release. So we build that using the Dockerfile in the `pdns` folder and the following command:
...
# Measurements
With our query replay tool and our containers of different nameserver implementation we could then perform some measurements.
...
If you want to perform the same measurements that we did, you must set up the machines you run them on with the same settings. 
...
so it does that 11 times). The only exception is the `run_nameserver_batch_only_tcp.sh` script, which runs the measurement where all the DNS requests are immediately sent over TCP for all nameservers. For this it uses the `no-retry/querier-no-retry.go` file instead of the `querier.go` file to send the DNS requests.
